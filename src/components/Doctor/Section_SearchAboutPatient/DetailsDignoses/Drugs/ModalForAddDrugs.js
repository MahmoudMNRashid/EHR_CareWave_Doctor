...
import classes from './ModalForAddDrugs.module.css'
...
const ModalAdd = (props) => {
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            const response = await fetch('http://localhost:8001/v1/Symptoms/Drug', {
                method: 'POST',
            ...
            if (data.data.message === 'Drugs added') {
    ...
    const configurationName = { type: 'text', label: 'اسم الدواء' }
    return (
        <div className="min-w-screen h-screen animated fadeIn faster  fixed  left-0 top-0 flex justify-center items-center inset-0 z-50 outline-none focus:outline-none bg-no-repeat bg-center bg-cover">
            <form className={classes.container} onSubmit={handleSubmit}>

                <div className={classes.title}>
                    <p> إضافة وصفة أدوية </p>
...
export const ModalForAddDrugs = (props) => {
