...
import classes from './ModaDiseasesVaccines.module.css'
import { InputDiseases } from '../UI_2/InputDiseases'
import { createPortal } from 'react-dom';
 const ModaDiseasesVaccines = () => {
    return (
        <div className="min-w-screen h-screen animated fadeIn faster  fixed  left-0 top-0 flex justify-center items-center inset-0 z-50 outline-none focus:outline-none bg-no-repeat bg-center bg-cover">
       
        <div className={classes.wrapper}>
            <div className={classes.first}>
                <h2>للتعديل أو الحذف </h2>
            </div>
            ...
            <div className={classes.third}>
                <div className={classes.buttonwrapper}>
                    <button className={classes.btn}> تعديل </button>
                    <button className={classes.btn}>الغاء </button>
                </div>
                <button className={`${classes.buttondelete} ${classes.tooltip}`}>
                    <svg
...
export const ModalDV = () => {
    return (
      
          createPortal(<ModaDiseasesVaccines/>, document.getElementById('modal'))
